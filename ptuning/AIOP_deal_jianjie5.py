...
print(nodes.shape) # (2220, 17, 6, 124)
...
nodes_prompt_list=[]
prompt_node_all=f'目前存在{node_nums}个节点,节点之间可能存在trace的关系,如果节点A和节点B相连,则我们用A-B表示,目前的节点连接关系表示为'
for node_ad in range(len(adjcent)):
    for node_index in range(len(adjcent[node_ad])):
        if adjcent[node_ad][node_index]==1:
            prompt_node=f'{value_name_dict[node_ad]}-{value_name_dict[node_index]},'
            prompt_node_all+=prompt_node
# 最后一个逗号改为句号
prompt_node_all=prompt_node_all[:-1]+'。'
# 训练的时候暂时不使用这部分提示，讲的时候要讲到这一部分
prompt_node_all=''
for index,(time,node_metric_timerange,label) in enumerate(list(zip(timestamp,nodes,labels_timestamp))): # nodes的shape为 (2220, 17, 6, 124),labels_timestamp的shape为（2220，）
    prompt_element=''
    prompt1=f'在时间窗口{time},共包含{len(node_metric_timerange[0])}个时刻:'
    prompt_element+=prompt1+prompt_node_all
    prompt23=''
    for i in range(len(node_metric_timerange)):
        ...
        for j in range(len(node_metric_timerange[i])):
            ...
            for index,(metric_value_now,metric_value_last) in enumerate(zip(metrics_values_list,metrics_last_list)):
                # 目前这版差值要大于10.0才进行统计
                if math.fabs(metric_value_last-metric_value_now)>10.0:
                    flag=1
            ...
            if flag==0:
                ...
                count+=1
                if count==1:
                    prompt3=f'{prompt2}第{j+1}时刻变化值'
                else:
                    prompt3=f'第{j+1}时刻变化值'
                up_value=f'{",".join(up_list)},' if len(up_list)>0 else ''
                if len(up_list)>0 and len(down_list)<=0:
                    up_value=up_value[:-1]+'。'
                down_value=f'{",".join(down_list)}。' if len(down_list)>0 else ''
                prompt_diff=f'{up_value}{down_value}'
                prompt3+=prompt_diff
            prompt23+=prompt3
    ...
    summary='该节点存在异常(1)' if label==1 else '该节点为正常状态(0)'
...
train_num_split_point=int(len(timestamp)*0.8)
...
for ob in nodes_prompt_list:
    if '1' in ob['summary']:
        print(ob['content'])
        print(ob['summary'])
        break
# print(train_nodes_prompt_list[0]['content'])
# print(train_nodes_prompt_list[0]['summary'])

# 减少一些训练集中正常样本数量，维持训练集中normal:abnormal的比例为1:5,但是在jianjie5中暂时不需要
# label_0_list=[]
# label_1_list=[]
...
with open('AIOPS/jianjie5/train_jianjie5.json','w',encoding='utf-8') as f:
...
with open('AIOPS/jianjie5/dev_jianjie5.json','w',encoding='utf-8') as f:
    for prompt_finish in dev_nodse_prompt_list:
        f.write(json.dumps(prompt_finish,ensure_ascii=False)+'\n')

print('已写入AIOPS/jianjie5/dev_jianjie5.json')
