...
prompt_prefix=f'现在总共有{node_nums}个节点,其中每个节点在某一个timestamp时刻,会存在一共{len(metric_name_list)}个metrics(指标)状态。'
...
prompt_metric=f"""我们将其记录为metric0,metric1,...一直到metric{len(metric_name_list)-1}。这些mtric可能为例如Buffers_used,CPU_free_pct等,值为保留两位小数后的浮点数。
接下来我将给出每个不同node在三个等间隔timestamp所形成的时间窗口下,所对应的每个timestamp相比于上一个timestamp的metrics变化值,如果没有指出某个metric则说明这个metric在这个timestamp中相比于上一个timestamp没有变化,我们将用这个时间窗口的metric变化情况推断该窗口下是否存在异常。
"""
    ...
    for i in range(len(node_metric_timerange)):
        ...
        for j in range(len(node_metric_timerange[i])):
            ...
            if flag==0:
                ...
                prompt_diff=f'上涨指标及其对应的值为:{",".join(up_list)},下降指标及其对应的值为:{",".join(down_list)}。'
                prompt3+=prompt_diff
...
with open('AIOPS/train_jianjie.json','w',encoding='utf-8') as f:
    for prompt_finish in train_nodes_prompt_list:
        f.write(json.dumps(prompt_finish,ensure_ascii=False)+'\n')

with open('AIOPS/dev_jianjie.json','w',encoding='utf-8') as f:
    for prompt_finish in dev_nodse_prompt_list:
        f.write(json.dumps(prompt_finish,ensure_ascii=False)+'\n')



with open('AIOPS/content_example_jianjie.txt','w',encoding='utf-8') as f:
